...
public class detallesVueloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (nombreVuelo != null && !nombreVuelo.isEmpty()) {
				...
				if (!validarVuelo(request, (VueloDto)request.getSession().getAttribute("datosVuelo"))) {
	...
	private boolean validarVuelo(HttpServletRequest request, VueloDto datosVuelo){
		RutaDeVueloDto datosRuta = serviceIUsuario.obtenerRutaDeVueloPorNombre(datosVuelo.getRuta());
		if (datosRuta.getEstado() != EstadoRuta.CONFIRMADA) {
			String tipo = (String)request.getSession().getAttribute("tipo");
			if (tipo.equals("aerolinea")) {
				String nicknameAero = (String)request.getSession().getAttribute("usuario");
				return nicknameAero.equals(datosVuelo.getAerolinea());
			}
